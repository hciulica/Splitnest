...
import { getStorage, uploadBytes, deleteObject, ref, getDownloadURL } from 'firebase/storage';
...
const SettingsScreen = ({navigation,}) => {
    ...
    const [verified, setVerified] = useState(authentication.currentUser.emailVerified);
    ...
    const passwordDisabled = (newPassword === null || newPassword === '') ? true : false;
    ...
    const resetPassword = () => 
    {
        ...
        if(verified){
            ...
            [
                {
                text: "Reset",
                onPress: newPassword => updatePassword(authentication.currentUser, newPassword)
                        .then(() => {
    ...
    const verificationEmail = () => {
        ...
        .then(() => {
            ...
            if(errorCode === 'auth/too-many-requests')
                Alert.alert('Please try again later', 'You sent to many requests');
        })
    }
    ...
    const handleDeleteAccount = () => {
        ...
        } else Alert.alert('Confirm email first' ,'Please send a confirmation email to enable account settings');
    ...
    return (
        ...
        <SafeAreaView style={styles.container}>
            <View style={{alignSelf: 'flex-start', alignItems: 'center', flexDirection: 'row', marginTop: 20}}>
                <BackButton style={{marginLeft: 26}} onPress={ () => backHandle() } />
                <Text style={{fontSize: 18,fontWeight: 'bold',marginLeft: 19}}>Settings</Text>
            </View>
            {/* AccountCard */}
            ...
            <FlipCard 
                ...
                flipHorizontal={true}
                flipVertical={false}
                flip = {(accountCardState === 1 || accountCardState === 2) ? true : false}
                clickable={false}
                ...
                <View style={styles.accountContainer}>
                    <Text style={{fontWeight: '500', marginLeft: 26, alignSelf: 'flex-start'}}>ACCOUNT</Text>
                
                <TouchableWithAnimation onPress={ () => resetHandle() }
                     duration={150} pressAnimation={0.97} 
                    style={{flexDirection: 'row', justifyContent: 'center', alignItems: 'center', marginTop: 35}}>
                        <KeyLogo/>
                            <View style={{marginLeft: 20, marginRight: 30}}>
                                <Text style={{fontWeight: '700', fontSize: 12, marginBottom: 4,}}>Change password</Text>
                                <Text style={{fontWeight: 'bold', fontSize: 10, color: 'rgba(151,151,151,0.9)'}}>Change password, reset password</Text>
                            </View>
                        <AngleLogo></AngleLogo>
                    </TouchableWithAnimation>

                    <TouchableWithAnimation onPress={() => handleDeleteAccount()}
                    duration={150} pressAnimation={0.97} style={{flexDirection: 'row', justifyContent: 'center', alignItems: 'center', marginTop: 35}}>
                        <DeleteLogo/>
                        <View style={{marginLeft: 20, marginRight: 30}}>
                            <Text style={{fontWeight: '700', color: '#FF0000', fontSize: 12, marginBottom: 4,}}>Delete account</Text>
                            <Text style={{fontWeight: 'bold', fontSize: 10, color: 'rgba(151,151,151,0.9)'}}>Remove your account permanently</Text>
                        </View>
                        <AngleLogo></AngleLogo>
                    </TouchableWithAnimation>
                </View>
                ...
                {/* Back Side */}
                    ...
                    <View style={{ alignItems: 'center', justifyContent: 'center'}}>
                        <Text style={{alignSelf: 'flex-start', marginBottom: 10,marginLeft: 5, fontSize: 13,}} >New password</Text>
                        <View style={{backgroundColor: 'rgba(49,101,255,0.1)', borderRadius: 10, width: 211, height: 40, justifyContent:'center', alignItems: 'center', flexDirection: 'row'}}>
            ...
            <View style={styles.securityContainer}>
                <Text style={{fontWeight: '500', marginTop: 33, marginLeft: 26}}>SECURITY</Text>
                
                <View duration={150} pressAnimation={0.97} style={{flexDirection: 'row', justifyContent: 'center', alignItems: 'center', marginTop: 35}}>
                    {!verified ? <EmailLogo/> : <ThickLogo/>}
                    <View style={{marginLeft: 20, marginRight: 30}}>
                        <Text style={{fontWeight: '700', fontSize: 12, marginBottom: 4,}}>Confirm email</Text>
                        <Text style={{fontWeight: 'bold', fontSize: 10, color: 'rgba(151,151,151,0.9)'}}>Send a confirmation email </Text>
                        <Text style={{fontWeight: 'bold', fontSize: 10, color: 'rgba(151,151,151,0.9)'}}>to enable account settings</Text>

                </View>
                    <FlatButton title="Send" onPress = { () => verificationEmail() } disabled={verified} height={30} width={50} fontSize = {10} ></FlatButton>
                </View>
