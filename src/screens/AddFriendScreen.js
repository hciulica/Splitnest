...
import { doc, onSnapshot , setDoc, getDoc, updateDoc, getDocs, collection, query } from 'firebase/firestore';
...
const AddFriendScreen = ({navigation}) => 
{
    ...
    useEffect (() => {
        ...
        const getUsersInfo = async() => {
            ...
            const results = users.filter(({email, username, image}) => 
                !friends.some(exclude => exclude.email === email && exclude.username === username && exclude.image === image)
            );
            setResults(results);
    ...
    const searchFilter = (text) => {
        if(text) {
            const newData = results.filter((item) => {
                ...
                const itemData1 = (item.email) ? (item.email.toUpperCase()) : ''.toUpperCase()
        ...
        <View style={styles.container}>
            <SafeAreaView style={styles.topContainer}>
                <View style={styles.topElements}>
                    <BackButton style={{marginLeft: 25}} onPress={() => navigation.goBack()}/>
                ...
                <View style={{flexDirection: 'row', justifyContent: 'center', alignItems: 'center', marginTop: 30}}>
                    ...
                    <FlatButton onPress={() => clearText()} style={{marginLeft: 20}} title="Clear" width={60} height={35} fontSize={12}></FlatButton>
                </View>
            ...
            {(filteredResults.length)? 
                ...
                /> :
                <Text style={{fontSize: 11,color: 'rgba(0,0,0,0.60)', marginTop: 60}}>No accounts to be displayed</Text>
            }
